...
I developed this project as part of my master thesis. This application allows users to creat and sign contracts or documents online with their Decentralized Identity (DID) and Verifiable Credential (VC). The users can choose to insert the verified personal information from the Verifiable credential into the contract document. This is different from a normal document signing when only your signature is a signature and the rest of the document are just plain texts. This time the important information you filled in the document, such as national ID, are the signature themselve if it come from the verifiable credentials. While the document itself is never exposed to the public, its integrity can still be verified and authenticated by anyone since the application leave a cryptographic proof on the blockchain when the documents are signed. Once the documents are signed, the terms of the contract are translated into smart contract and deployed to Ethereum blockchain for enforcement. For more information, refer to the pdf file link aboved. 

Note that this repository is for a concept demo to showcase the use case of blockchain in contract signing, you can not just clone this project and use it, since it required a verifiable credential that is issued by me (the only trusted issuer recognized by the application), which I do not have a server up running to issue people anymore. In real-world application, you can chose to accept any verifiable credential from any issuer, at the time of my thesis (2022), the concept of verifiable credential is still new and developing, so I could not write code to accept ANY verifiable credential yet.
